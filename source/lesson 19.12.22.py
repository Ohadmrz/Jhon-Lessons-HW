...
#     print(run_time)
############################################################################################
# ~~~ C10 - datetime (exercise) ~~~~
#
# (1)
# Implement a function that gets a string that represents datetime in the following format: "2021-12-08, Wed, 10:00"
# and returns the name of the weekday 3 days after the received date.
# For example, for the input provided, the output should be: Saturday.
# Write 2 unit tests to test your function
import datetime

def get_date_in_3_days(original_date: str):        # הפונקציה מקבלת סטרינג שהוא התאריך והזמן דייטטיים  - original_date
    dt = datetime.datetime.strptime(original_date, "%Y-%m-%d, %a, %H:%M")   # dt: datetime.datetime = datetime.datetime.strptime(original_date,"%Y-%m-%d, %a, %H:%M")
    new_dt = dt + datetime.timedelta(days=3)                                # מוסיף 3 ימים לתאריך שקיבלתי בשורה הקודמת
    return new_dt.strftime("%A")                                            # מבקש לקבל בחזרה רק את הסטרינג של היום בשבוע מתוך התאריך שיש לי. קבעתי שהסטרינג שהפונקציה תמיר יהיה רק היום בשבוע.
...
#
# (4)
# Implement a function that receives a time in format hh:mm
# that represents the time when the lecture ends, and returns the amount of time left to the end of the lecture.
